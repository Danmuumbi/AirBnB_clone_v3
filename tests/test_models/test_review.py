...
class TestReviewDocs(unittest.TestCase):
    ...
    def test_pycode_conformance_review(self):
        """Test that models/review.py conforms to PEP8."""
        pycode_s = pycodestyle.StyleGuide(quiet=True)
        result = pycode_s.check_files(['models/review.py'])
        self.assertEqual(result.total_errors, 0,
                         "Found code style errors (and warnings).")

    def test_pycode_conformance_test_review(self):
        """Test that tests/test_models/test_review.py conforms to PEP8."""
        pycode_s = pycodestyle.StyleGuide(quiet=True)
        result = pycode_s.check_files(['tests/test_models/test_review.py'])
        self.assertEqual(result.total_errors, 0,
                         "Found code style errors (and warnings).")
