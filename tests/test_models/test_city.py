...
class TestCityDocs(unittest.TestCase):
    ...
    def test_pycode_conformance_city(self):
        """Test that models/city.py conforms to PEP8."""
        pycode_s = pycodestyle.StyleGuide(quiet=True)
        result = pycode_s.check_files(['models/city.py'])
        self.assertEqual(result.total_errors, 0,
                         "Found code style errors (and warnings).")

    def test_pycode_conformance_test_city(self):
        """Test that tests/test_models/test_city.py conforms to PEP8."""
        pycode_s = pycodestyle.StyleGuide(quiet=True)
        result = pycode_s.check_files(['tests/test_models/test_city.py'])
        self.assertEqual(result.total_errors, 0,
                         "Found code style errors (and warnings).")
