...
class TestAmenityDocs(unittest.TestCase):
    ...
    def test_pycode_conformance_amenity(self):
        """Test that models/amenity.py conforms to PEP8."""
        pycode_s = pycodestyle.StyleGuide(quiet=True)
        result = pycode_s.check_files(['models/amenity.py'])
        self.assertEqual(result.total_errors, 0,
                         "Found code style errors (and warnings).")

    def test_pycode_conformance_test_amenity(self):
        """Test that tests/test_models/test_amenity.py conforms to PEP8."""
        pycode_s = pycodestyle.StyleGuide(quiet=True)
        result = pycode_s.check_files(['tests/test_models/test_amenity.py'])
        self.assertEqual(result.total_errors, 0,
                         "Found code style errors (and warnings).")
